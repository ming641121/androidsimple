...
#endif
int sysCraftBaudRate = 38400;

int SMCMR_STD_MODE    =	(SMCMR_CLEN_9_BITS | \
				 SMCMR_SL_1_BIT    | \
				 SMCMR_SM_UART	   | \
				 SMCMR_DM_NORMAL);
...
STATUS smc8260Ioctl (PPC8260SMC_CHAN *pChan,int request,int arg);
void   smc8260ResetChannel (PPC8260SMC_CHAN *pChan);
...
#endif
    pChan->baudRate  = sysCraftBaudRate;
...
/******************************* BSP ADD **********************************/
    ...
    cpcrVal = (M8260_CPCR_OP (M8260_CPCR_TX_STOP)
               | M8260_CPCR_SBC (M8260_CPCR_SBC_SMC2)
               | M8260_CPCR_PAGE (M8260_CPCR_PAGE_SMC2)
               | M8260_CPCR_FLG);
...
STATUS smc8260ChangeOption( PPC8260SMC_CHAN *pChan )
{
    int immrVal = vxImmrGet();
    int oldlevel = intLock ();	/* lock interrupts */ 
...
*
* This routine recive data from the SMC channel in polling mode.
*
