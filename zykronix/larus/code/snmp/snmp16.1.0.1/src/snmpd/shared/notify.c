...
SrInitializeTraps()
{
    ...
    if (!ipTransportOpened) {
        ...
        if (OpenTransport(&ipTrapTI)) {
            DPRINTF((APWARN, "%s: cannot open IP transport traps.\n", Fname));
            initialized = -1;
            return -1;
        }
        ipTransportOpened = 1;
        ...
        if ((LocalIP == -1) || (LocalIP == 0)) {
            DPRINTF((APWARN, "%s: host db doesn't know self.\n", Fname));
            initialized = -1;
            return -1;
        } else {
        ...
        case SR_SNMPv1_VERSION:
            if ((*v1_pdu) == NULL) {
                *v1_pdu = SrConvertV2NotificationToV1Notification(
                    *v2_pdu, systemData.sysObjectID, &localAddr);
                if ((*v1_pdu) == NULL) {
                    return -1;
                }
            }
        ...
        case SR_SNMPv1_VERSION:
            if ((*v1_pdu) == NULL) {
                if (agent_addr == NULL) {
                    ...
                    *v1_pdu = SrConvertV2NotificationToV1Notification(
                        *v2_pdu, systemData.sysObjectID, agent_addr);
                }
                if ((*v1_pdu) == NULL) {
                    return -1;
                }
    ...
    TransportInfo *dstTI)
{
    ...
    if (SendToTransport((char *) snmp_msg->packlet->octet_ptr,
                        (int) snmp_msg->packlet->length, ti) < 0) {
        DPRINTF((APWARN,
