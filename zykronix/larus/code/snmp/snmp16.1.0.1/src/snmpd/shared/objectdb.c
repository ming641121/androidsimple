...
#ifdef SR_SNMPv2_PDU
int
IsAggregateType(int type)
{
    return 0;
}
#else
#define IsAggregateType(x) 0
#endif /* SR_SNMPv2_PDU */
    ...
    int             i, payload;
{
    ...
    while (i < TotalObjects) {
	   ...
	   i++;
	} else if ( (payload != SR_AGGREGATE_PDU_PAYLOAD) && 
#else                           /* SR_SNMPv1_PACKET not defined */
        if ( (payload != SR_AGGREGATE_PDU_PAYLOAD) && 
#endif				/* SR_SNMPv1_PACKET */
             (IsAggregateType((int)(OidElem(i)->oidtype)))) {

           /* if payload is not an aggregate PDU skip aggregate objects */
           i++;
        }
        else {
          break;
    ...
    int             searchType, payload;
{
    ...
    if ((payload != SR_AGGREGATE_PDU_PAYLOAD) ) {
#endif				/* SR_SNMPv1_PACKET */
	if(searchType == EXACT && IsAggregateType((int)(OidElem(i)->oidtype))) {
	   DPRINTF((APTRACE, "%s: searchType exact on aggregate object in non aggregate PDU\n", Fname));
	   return -1;
	}
	while (i < TotalObjects &&
	       IsAggregateType((int)(OidElem(i)->oidtype))) {
    ...
    int             serialNum, payload;
{
	...
	i = NextObjectClass(i, payload);
        ...
        if ((i != -1) &&
            ((OidElem(i)->oidstatus & FULLY_QUALIFIED) != FULLY_QUALIFIED)) {
              local_incoming = &(OidElem(i)->oid);
        }
