    ...
    ContextInfo    *contextInfo;
{
    ...
    int i;
    SR_UINT32 tmp_ip_addr = ((SR_INT32)0);
    ...
    case I_ipRouteNextHop:
       ...
       */
        ...
        for (i = 0; i < 4; i++) {
            tmp_ip_addr <<= 8;
            tmp_ip_addr += (SR_INT32) (value->os_value->octet_ptr[i]);
        }

        if (tmp_ip_addr == ((SR_INT32)0)) {
            dp->state = SR_DELETE;
            return NO_ERROR;
        }
	...
	if (value->sl_value != D_ipRouteType_indirect) {
            if (value->sl_value != ((ipRouteEntry_t *) (dp->data))->ipRouteType) {
	        return (WRONG_VALUE_ERROR);
            }
	}
