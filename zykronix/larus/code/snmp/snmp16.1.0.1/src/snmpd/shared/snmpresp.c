    ...
    int             payload;
{
     ...
     */
    if ((outpdu = MakePdu(GET_RESPONSE_TYPE, req, (SR_INT32) NO_ERROR, ((SR_INT32)0),
		 (OID *) NULL, (OctetString *) NULL, ((SR_INT32)0), ((SR_INT32)0), ((SR_INT32)0))) == NULL) {
	...
	for (invb = inpdu->var_bind_list; invb; invb = invb->next_var) {
	    ...
	    total_so_far += (SR_UINT32)(outvb->name_size + outvb->value_size+4);

	    /* 4 <-- 1 tag, 1 definite long 2, 2 length (worst case) */
	     if (total_so_far >= limit - 4) { 
