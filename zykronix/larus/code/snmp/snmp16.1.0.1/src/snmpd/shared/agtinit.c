...
#ifdef HAVE_SYS_UTSNAME_H
  {
     ...
     if (uname(&unamebuf) < 0) {
        ...
        if (descr) {
            ...
            sprintf(descr, "%s release:%s version:%s machine:%s",
    ...
    SnmpLcd *snmp_lcd)
{
...
#if defined(SR_SNMPv3_ADMIN)
    agt_local_snmpID = SrGetSnmpID(SR_SNMPID_ALGORITHM_SIMPLE,
                                   SR_SNMPID_ALGORITHM_SIMPLE_AGT);
    ...
    agt_snmp_v3_lcd.usm_salt =
        ((SR_UINT32)(long)snmp_lcd) * ((SR_UINT32)GetTimeNow());
...
terminate(void)
{
     ...
     * k_system_initialize() or k_srAgentInfo_initialize().
     */
