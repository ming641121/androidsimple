    ...
    NotificationRecord *nr)
{
...
#ifdef SR_IP
    if (nr->ti.type == SR_IP_TRANSPORT) {
        ipTrapTI.t_ipAddr = nr->ti.t_ipAddr;
        if (nr->ti.t_ipPort == 0) {
            ipTrapTI.t_ipPort = htons((short)GetSNMPTrapPort());
        } else {
    ...
    if (SendToTransport((char *) nr->snmp_msg->packlet->octet_ptr,
                        (int) nr->snmp_msg->packlet->length, ti) < 0) {
        DPRINTF((APWARN,
    ...
    TransportInfo *ti)
{
    ...
    if (trap_pkt == NULL) {
        trap_pkt = (unsigned char *)malloc(2048);
        if (trap_pkt == NULL) {
            return;
        }
    }

    length = ReceiveFromTransport((char *)trap_pkt, 2048, ti);
    if (length <= 0) {
        return;
    }
    else {
    ...
    SR_INT32 cfg_chk)
{
    ...
    if ((only_snmp_msg != NULL) && (only_ti != NULL) && (cfg_chk == 0)) {
        ...
        if (convert_pdu(only_snmp_msg->version, &v1_pdu, &v2_pdu)) {
            goto done;
        }
...
#if (defined(SR_SNMPv1_PACKET) && defined(SR_SNMPv2_PDU))
            /* Perform pdu conversion if necessary */
            if (convert_pdu(stpe->snmpTargetParamsMPModel, &v1_pdu, &v2_pdu)) {
                continue;
            }
#endif	/* (defined(SR_SNMPv1_PACKET) && defined(SR_SNMPv2_PDU)) */
    ...
    SR_INT32 timeout)
{
    ...
    if ((snmp_msg != NULL) && (only_ti != NULL) && (cfg_chk == 0)) {
#if (defined(SR_SNMPv1_PACKET) && defined(SR_SNMPv2_PDU))
        /* Perform pdu conversion if necessary */
        if (translate_pdu(snmp_msg->version, &v1_pdu, &v2_pdu, agent_addr)) {
            goto done;
        }
#endif	/* (defined(SR_SNMPv1_PACKET) && defined(SR_SNMPv2_PDU)) */
        if (notify_serial_num == 0x7fffffff) {
        ...
        switch (snmp_msg->version) {
            ...
            case SR_SNMPv3_VERSION:
#endif /* SR_SNMPv3_PACKET */
                v2_pdu->u.normpdu.request_id = notify_serial_num;
                if (notifyType == SR_NTFY_TYPE_INFORM) {
#ifdef SR_NOTIFY_FULL_COMPLIANCE
                    v2_pdu->type = INFORM_REQUEST_TYPE;
                    Sr_init_inform(notify_serial_num,
...
#if (defined(SR_SNMPv1_PACKET) && defined(SR_SNMPv2_PDU))
            /* Perform pdu conversion if necessary */
            if (translate_pdu(stpe->snmpTargetParamsMPModel, &v1_pdu, &v2_pdu,
                              agent_addr)) {
                continue;
            }
#endif	/* (defined(SR_SNMPv1_PACKET) && defined(SR_SNMPv2_PDU)) */
