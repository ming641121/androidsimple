    ...
    SrGetInfoEntry *info_entry)
{
    ...
    static SrGetInfoEntry *last_info = NULL;
    ...
    static short      alloc_data_size = 0;
    int               idx_olen = 0;
    static SR_UINT32 *alloc_oid_ptr = NULL;
    static short      alloc_oid_ptr_size = 0;
    ...
    if (incoming == NULL || object == NULL ||
            ...
            contextInfo == NULL || info_entry == NULL) {
        /*
    ...
    if((serialNum == last_serialNum) && (serialNum != -1) && 
        ...
        if ((NEXT_ROW(cache_valid_ptr)) && 
            (VALID(object->nominator, cache_valid_ptr))) {
            goto data_in_cache;
        }
    }
    /*
    ...
    if (info_entry->index_info == NULL) {
        if (CheckScalarInstance(incoming, new_object, searchType) < 0) {
            goto v_get_no_data;
        }
        ...
        switch(ExtractIncomingIndex(incoming,
                                 instLength,
                                 info_entry,
                                 searchType,
                                 new_object,
