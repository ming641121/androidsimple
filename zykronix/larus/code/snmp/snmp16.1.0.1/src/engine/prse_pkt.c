    ...
    const SR_INT32 packet_len)
{
    ...
    if ((pdu_type = ParseType(&working_ptr, end_ptr)) == -1) {
        DPRINTF((APPACKET, "SrParsePdu, pdu_type\n"));
        goto fail;
    }
    ...
    default:
        DPRINTF((APPACKET, "SrParsePdu, bad pdu_type: %x\n", pdu_type));
        goto fail;
    };                                /* end of switch */
    ...
    short                *type;
{
    ...
    /* SLM:
     *      ?? OKay to check if this is a sequence by checking
     *      type & SEQUENCE_TYPE or do I need to check all possible
     *      sequence types here?
     */
