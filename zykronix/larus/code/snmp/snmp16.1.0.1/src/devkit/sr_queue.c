...
int SRQDump(SRQueue *queue, void (*PtrPrintFcn)(void *ptr))
{
    ...
    /* --- Return if the queue is not initialized --- */
    if (queue->initialized != SR_QUEUE_INITIALIZED) {
        printf("SRQDump:  Queue 0x%08X is not initialized.\n", (int)(long) queue);
        return 0;
    }

    /* --- Lock and/or wait on the queue --- */
    if (SRQLock(queue) == -1) {
        printf("SRQDump:  Queue 0x%08X could not be locked.\n", (int)(long) queue);
        return -1;
    }
    ...
    printf("SRQDump:  Queue 0x%0X:  %d entries, head=0x%0X, tail=0x%0X.\n", 
            (int)(long) queue, (int)(long) queue->count, (int)(long) head, (int)(long) tail);
