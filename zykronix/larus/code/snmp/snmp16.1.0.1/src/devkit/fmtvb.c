...
{
   /*
    *  OID_Descriptor is a characger buffer where the human-friendly OID
    ...
    *  NULL character.
    */
   int           bufsize_needed, oidsize, pos, rc, valuesize;
   ...
   /*
    *  If the caller wants to return the result in a buffer, and the
    *  caller provided the buffer, return if the buffer is too small.
    */
   if (((behavior & SR_FMTVB_BUFFER) == SR_FMTVB_BUFFER) &&
        (*buffer != NULL) && (buflen < bufsize_needed)) {
       return -1;
   }
   ...
   /* --- Malloc a buffer in which to build the result --- */
    ...
    *  SprintVarBindValue() to render the value.  Do some pointer 
    *  funniness so that we don't have to malloc a second buffer.
    */
   pos = strlen(result);
   rc  = FPrintVarBindValue(vb, &(result[pos+3]), OID_Descriptor, bufsize_needed-(pos+3));
   ...
   if ((behavior & SR_FMTVB_BUFFER) == SR_FMTVB_BUFFER) {
       ...
       if (*buffer != NULL) {
           /* --- Copy result into the user-supplied buffer --- */
