...
#define SR_NO_WRITE_CONFIG_FILE
  ...
  const char *backup_filename;
{
    ...
    if ((fp = fopen(filename, "r")) == NULL) {
        DPRINTF((APWARN, "%s:  Error opening %s: %s\n", Fname, filename,
          sys_errname(errno)));
        return 0;
    }
    if ((fp2 = fopen(backup_filename, "w")) == NULL) {
        DPRINTF((APWARN, "%s: Error opening %s: %s\n", Fname, 
           backup_filename, sys_errname(errno)));
        fclose(fp);
        return 0;
    }
    while (fgets((char *) buffer, 256, fp) != NULL) {
        (void) snprintf(templine, sizeof(templine), "%s", buffer);
        fputs(templine, fp2);
    }
    fclose(fp2);
    fclose(fp);
