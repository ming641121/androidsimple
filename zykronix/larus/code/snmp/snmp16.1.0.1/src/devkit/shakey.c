...
/* this routine produces a key using the SHA hash algorithm on a password
 * which is repeated and extended to 1 Mbyte in size */
OctetString *
MakeSHAPasswordKey(password)
const OctetString *password;
{
    ...
    long count;
    OctetString *retval;
    unsigned char buf[SHA_DIGEST_SIZE];
	...
	/* update the digest */
        SHAUpdate(&SHA, pw_buf, sizeof(pw_buf));
    }

    /* finish calculating */
    SHAUpdate(&SHA, pw_buf, 0);
    SHAFinal(buf, &SHA);
    ...
    const OctetString *key)
{
    ...
    SHAInit(&SHA);
    SHAUpdate(&SHA, key->octet_ptr, key->length);
    SHAUpdate(&SHA, snmpEngineID->octet_ptr, snmpEngineID->length);
    SHAUpdate(&SHA, key->octet_ptr, key->length);
    SHAFinal(buf, &SHA);
